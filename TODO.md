...
- [🔴] launch-role! runs ensure-codex-trust! for a `deepseek` role (not just literal `codex`), so a deepseek-backed role's codex process doesn't hang forever on codex's first-run "trust this directory?" dialog with no one there to answer it.
