...
- [🟢] check-backend-dependencies! checks for the `codex` binary (not a nonexistent `deepseek` binary) when a role's backend is `deepseek`, so startup doesn't fail with "'deepseek' is required but not installed" even though codex is present.
